  ...
  const socialButtons = [
    {
      ...
      url: `https://wa.me/?text=${encodeURIComponent( frontendUrl)}`,
    ...
    },
    {
      ...
      bgColor: "bg-gradient-to-br from-[#0A66C2] to-[#004182]",
      textColor: "text-white",
    },
  ]
    ...
    <Dialog>
      ...
      <DialogContent isOpen={isOpen} onClose={handleClose} className="sm:max-w-md">
        ...
        <div className="grid grid-cols-2 gap-6 p-6">
          {socialButtons.map(({ name, icon, url, bgColor, textColor }) => (
            <div key={name} className="flex flex-col items-center">
              <a
                href={url}
                target="_blank"
                rel="noopener noreferrer"
                className={`flex items-center justify-center w-16 h-16 rounded-full ${textColor} ${bgColor} shadow-md
                  transition-all duration-300 transform hover:scale-110 hover:shadow-lg cursor-pointer`}
                onClick={(e) => {
                  e.preventDefault()
